...
def train(data_path: Path, ft_hidden: int = 256, hidden1_size: int = 32, epochs: int = 10, batch_size: int = 1024, lr: float = 1e-3):
    device = torch.device("mps" if torch.backends.mps.is_available() else "cpu")
    print(f"training on {device}")

    model = NnueModel(ft_hidden, hidden1_size).to(device)
...
def main():
    ...
    train(args.data, args.ft_hidden, args.hidden1, args.epochs, args.batch, args.lr)
